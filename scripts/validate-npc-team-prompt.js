...
if (!m) {
  ...
  if (!/任何模式不得静默跳过/.test(prompt))
    errors.push('缺少「暂停点不得静默跳过」门禁（铁律 6：无论接力/全程/自动连续模式，到 CP1/CP2/CP3 必须停下等用户命令）');
  // 改造点 2：未设暂停点自动接续
  if (!/未设.{0,40}暂停点的步骤.{0,40}自动触发下一步|未设暂停点的步骤自动执行下一步/.test(prompt))
    errors.push('缺少「未设暂停点自动接续」门禁（运行模式：未设 ⏸CP 的步骤自动触发下一步，用户可在暂停点确认/纠正/停止）');
  // 改造点 3：每步留痕粒度强化（评审棒/修复棒/复评棒独立留痕，缺任一步即视为假装执行）
  if (!/评审-修复循环中，评审棒\/修复棒\/复评棒每一步各自独立留痕，缺任一步即视为假装执行/.test(prompt))
    errors.push('缺少「每步留痕粒度强化」门禁（铁律 7：评审棒/修复棒/复评棒每一步各自独立留痕，缺任一步即视为假装执行）');
  // 改造点 5：评审范围纪律（只评 PR diff 新增/改动行，不泛化）
  if (!/只评审 PR diff 新增.{0,20}改动行|只评 PR diff 新增.{0,20}改动行/.test(prompt))
    errors.push('缺少「评审范围纪律」门禁（铁律 15/CR 卡片：只评审 PR diff 新增/改动行，不得评审历史代码/无关文件/发表与 diff 无关的泛化意见）');
  // 改造点 5 联动：RES 定位问题只能针对 PR diff 范围内代码
  if (!/RES 定位问题只能针对 PR diff 范围/.test(prompt))
    errors.push('缺少「RES 定位范围纪律」门禁（铁律 15：RES 定位问题只能针对 PR diff 范围内的代码，不得借调研之名泛化到无关模块）');
  // 改造点 4：新增🔬研究员 RES 角色（角色卡片已 7 张强校验，此处再校验 RES 介入流水线）
  if (!/研究员 RES：需求背景调研|🔬 研究员 RES/.test(prompt))
    errors.push('缺少「研究员 RES 角色」声明（第 7 位专家：需求背景调研、问题根因定位、方案可行性研究）');
  // 改造点 6：N 轮评审-修复循环验收标准（有效下限 = max(10, 用户指定 N)）
  if (!/有效下限 = max\(10, 用户指定 N\)/.test(prompt))
    errors.push('缺少「N 轮评审-修复验收标准」门禁（铁律 8：有效下限 = max(10, 用户指定 N)，可核实验收）');
  ...
  const relayModeSection = sliceSection('【运行模式（每次调用必须先自检）】', '【铁律（最高优先级）】');
  if (
    relayModeSection !== null &&
    !/接力模式（默认、推荐）：用户每次召唤你，你\*\*只执行流水线中的一个步骤\*\*/.test(relayModeSection)
  )
    errors.push('运行模式缺少「接力模式（默认、推荐）」声明（每次召唤须默认只执行一步并输出接力卡）');
  if (relayModeSection === null)
    errors.push('运行模式段落不可达：无法校验「评审-修复循环自动连续执行」例外声明（运行模式/铁律锚点缺失或顺序错乱）');
  if (
    relayModeSection !== null &&
    !relayModeSection.includes('评审-修复循环自动连续执行')
  )
    errors.push(
  ...
  if (
    ...
    (!smokeCSection.includes('同一次召唤内自动连续执行') ||
      ...
      !/已清零（且已达有效下限轮次 max\(10, 用户要求 N\)）→ 独立召唤 6\/12 测试/.test(smokeCSection) ||
  ...
  if (smokeESection !== null && !/⏸CP1\/⏸CP2\/⏸CP3 暂停确认与留痕门禁仍生效/.test(smokeESection))
    errors.push(
      '冒烟测试方法 E 缺少全程模式边界约束（须含：一次跑完全部步骤时 ⏸CP1/⏸CP2/⏸CP3 暂停确认与留痕门禁仍生效）'
    );
