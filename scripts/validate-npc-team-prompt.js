...
if (!m) {
  ...
  if (!/需求覆盖度检查/.test(prompt))
    errors.push('缺少「需求覆盖度检查」门禁（6/12 测试通过后须逐条核对开发落点/测试覆盖是否覆盖全部需求）');
  if (!/逐条核对开发落点.{0,30}测试覆盖/.test(prompt) && !/开发落点.{0,20}测试覆盖/.test(prompt))
    errors.push('缺少「需求覆盖度矩阵」门禁（须核对开发落点与测试覆盖，输出覆盖度矩阵）');
  ...
  if (!/【暂停·CP2.5 需求覆盖确认】/.test(prompt))
    errors.push('缺少暂停卡模板「【暂停·CP2.5 需求覆盖确认】」（全程模式需求覆盖确认须有输出模板）');
