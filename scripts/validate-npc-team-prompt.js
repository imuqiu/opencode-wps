...
if (!m) {
  ...
  // ---- 5. 阶段流水线一致性（12 阶段，0/11 ~ 11/11）----
  if (prompt.includes('0/11') || prompt.includes('11/11')) {
    // 若使用 0/11 编号，则必须 0..11 全部出现
    for (let i = 0; i <= 11; i++) {
      if (!prompt.includes(`${i}/11`)) errors.push(`阶段编号 ${i}/11 缺失（12 阶段流水线不完整）`);
    }
  } else if (prompt.includes('阶段 1/') || prompt.includes('阶段 0/')) {
    errors.push('阶段编号分母不是 11（历史 bug：10 阶段/0/10 分母错误）');
  } else {
    warnings.push('未检测到 0/11~11/11 阶段编号（若已改为其他计数方式请确认一致性）');
  }
  ...
  // 三个暂停点必须以完整定义存在：⏸CP1 计划确认 / ⏸CP2 开发确认 / ⏸CP3 合并确认
  // （用完整词而非子串，避免「到 ⏸CP1/⏸CP2/⏸CP3 时输出暂停卡」这类引用句被误算为暂停点声明）
  ...
  if (!prompt.includes('⏸CP3 合并确认'))
    errors.push('缺少暂停点「⏸CP3 合并确认」（合并 PR 前人工确认机制缺失）');
  // 四个命令词必须以完整形式存在（「继续」「补充：<意见>」「停止」「继续合并」），防止子串误放行
  const cmds = [
    { name: '「继续」', re: /「继续」/ },
  ...
  for (const c of cmds) {
    if (!c.re.test(prompt)) errors.push(`缺少用户命令词 ${c.name}（暂停确认命令不完整）`);
  }
  ...
  // ---- 10c. 合并确认（⏸CP3，Issue #76 最新补充：PR 合并前暂停待用户确认）----
  // 背景：用户要求"再加上PR合并（在合并前需暂停待用户确认）"。
  // 因此 5/11 评审清零 + 6/11 测试通过 + 7/11 文档后、8/11 合并 PR 前，必须暂停输出【合并确认卡】
  // 等用户回复「继续合并」才执行合并；收到确认前绝不执行合并，禁止代替用户确认、禁止未经确认自行合并。
  // 本节强校验：若「合并前暂停确认」核心要素被删，CI 拦截（防偷偷合并 PR）。
  const MERGE_CONFIRM_CORE = [
    '13. 合并确认（⏸CP3）',
    '合并 PR 前必须暂停',
    '禁止代替用户确认、禁止未经确认自行合并 PR',
    '【合并确认卡（⏸CP3，5/11 评审清零 + 6/11 测试通过后、8/11 合并 PR 前输出）】',
    '合并 PR 前暂停，等用户回复命令词，收到前**绝不执行合并**',
    '「继续合并」→ 确认可合并，执行 8/11 合并 PR',
  ];
  let mergeLastIdx = -1;
  for (const frag of MERGE_CONFIRM_CORE) {
    const idx = prompt.indexOf(frag);
    if (idx === -1)
      errors.push(`合并确认缺少完整句式「${frag}」（合并 PR 前必须暂停等用户确认，防偷偷合并）`);
  ...
  // ---- 10d. 发布三要素（Issue #76 最新补充：更新版本号 + CHANGELOG + 发布产物）----
  // 背景：用户要求"发布（更新版本号、发布形成changelog、发布产物）"。
  // 因此 9/11 发布必须真实执行三要素：① 更新版本号（semver 递增）② 形成/更新 CHANGELOG ③ 发布产物（tag/release/制品），
  // 并在 Issue/PR 留痕；禁止假装发布（不更新版本号/不写 changelog/不产出发布产物却宣称已发布）。
  // 本节强校验：若「发布三要素」被删，CI 拦截（防假装发布）。
  const RELEASE_CORE = [
    '14. 发布三要素',
    '① 更新版本号',
    '② 形成/更新 CHANGELOG',
  ...
  for (const frag of RELEASE_CORE) {
    const idx = prompt.indexOf(frag);
    if (idx === -1)
      errors.push(
        `发布三要素缺少完整句式「${frag}」（发布须真实执行版本号+CHANGELOG+产物，防假装发布）`
      );
    else if (idx < releaseLastIdx)
      errors.push(
        `发布三要素句式顺序错乱：「${frag}」出现在其声明顺序之前（疑似插入干扰文本拆解语义）`
      );
  ...
  if (
    ...
    (!relayCardSection.includes('- 下一步召唤话术（用户原样复制即可）：') ||
      !/@CodeBuddy 接力 NPC_TEAM skill，执行下一步 N\+1\/11 <阶段名>：/.test(relayCardSection))
  )
    errors.push(
      '接力卡缺少「下一步召唤话术」（含标签与示例 @CodeBuddy 接力 NPC_TEAM skill，执行下一步 N+1/11 <阶段名>：）'
    );
  ...
  for (const { f, scope } of searchTargets) {
    ...
    if (idx === -1)
      errors.push(
        `评审-修复接力缺少完整句式「${f}」（5/11 评审-修复循环须逐轮接力，每轮评审/修复各为独立召唤，防假装进行）`
      );
  ...
  if (
    ...
    (!reReviewCardSection.includes('- 下一步召唤话术（用户原样复制即可）：') ||
      ...
      !/@CodeBuddy 接力 NPC_TEAM skill，执行 6\/11 测试（评审问题已清零，进入测试阶段）：/.test(
        reReviewCardSection
      ))
  )
    errors.push(
      '复评接力卡缺少「下一步召唤话术」（含标签与示例：执行第 R+1/10 轮评审（复评仍有问题）/ 执行 6/11 测试（问题已清零））'
    );
}
