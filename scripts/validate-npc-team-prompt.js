...
if (!m) {
  ...
  if (!cardSection) {
    ...
    // R6-1 修复：QA 卡片需求覆盖度检查职责（与主链 7/12、工作流程同步），
    // 防 QA 角色卡片未同步需求覆盖度/⏸CP2.5 职责而 CI 不拦截（角色职责与执行流程脱节）。
    if (!/QA.*需求覆盖度检查/s.test(cards))
      errors.push('角色卡片 QA 缺少需求覆盖度检查职责（须含：测试通过后做需求覆盖度检查）');
  }
  ...
  // ---- 5. 阶段流水线一致性（12 阶段，1/12 ~ 12/12）----
  // CR 第 11 轮修复（追加 9 轮循环 R1）：原 includes 子串校验存在双重逃生口——
  // ① includes('1/12') 命中 11/12、includes('2/12') 命中 12/12 的子串；
  // ② 即便改词边界正则，删除【流水线】段节点后仍会被暂停确认卡等其它段的引用字面量（已完成：1/12 需求接收 → 2/12 拆解分派）掩盖。
  // 因此改为：限定在【流水线】段内做 1..12 **全序出现**校验（词边界正则 + indexOf 递增），
  ...
  if (pipelineSectionStart === -1) {
    ...
    if (pipelineSectionEnd === -1 || pipelineSectionEnd <= pipelineSectionStart) {
      ...
      // 若使用 1/12 编号，则必须在流水线段内 1..12 全部出现且按序递增
      // CR 第 20 轮修复（追加 12 轮循环 R1）：去掉「边界标记进入条件」逃生口——
      // 原 if (stageIdRe(1) || stageIdRe(12)) 在 1/12 与 12/12 同时被删时条件不成立，
      // 走 else 只发 warning，12 阶段流水线删掉首尾两阶段仍放行（实测 exit 0）。
      // 改为无条件执行 1..12 全序校验：缺失任何阶段（含首尾）都报 error。
      {
        let lastIdx = -1;
        for (let i = 1; i <= 12; i++) {
          ...
          if (mm.index < lastIdx) {
            errors.push(`阶段编号 ${i}/12 顺序错乱（【流水线】段节点未按 1/12→12/12 递增排列）`);
            break;
          }
          lastIdx = mm.index;
        }
  ...
  // 说明：当前提示词已改 1/12~12/12 编号且评审轮次改为「第 R 轮」描述，R/10、R+1/10 模式不再出现，
  // 但保留此剔除作为防御性逻辑——防止未来旧编号重新引入或历史计轮描述重新出现时误判为 0/10~9/10 残留。
  const withoutReviewRounds = prompt.replace(/R\+1\/10/g, '').replace(/R\/10/g, '');
  for (let i = 0; i <= 9; i++) {
    if (new RegExp(`(?:^|[^0-9/])${i}\\/10(?:[^0-9]|$)`).test(withoutReviewRounds))
      errors.push(`阶段编号 ${i}/10 残留（已迁移为 ${i + 1}/12 或 ${i + 3}/12，历史编号未清理）`);
  }
  ...
  if (!/【暂停·CP2 开发确认】\n已完成：1\/12→5\/12 开发（PR 已建）/.test(prompt))
  ...
  // 补充（Issue #147 用户补充）：RES 角色介入 6/12——RES 须独立执行研究员视角评审并单独留痕，缺独立留痕即视为 RES 未介入（假介入）
  if (!/RES 6\/12 介入必须独立执行研究员视角评审并单独留痕/.test(prompt))
    errors.push(
      '缺少「RES 6/12 介入独立留痕」声明（角色卡片/工作流程：RES 独立执行研究员视角评审并单独留痕，缺独立留痕即视为 RES 未介入）'
    );
  if (!/「🔬 RES 研究员评审」独立留痕于 PR/.test(prompt))
    errors.push(
      '缺少「RES 研究员评审独立留痕锚点」门禁（RES 在 6/12 的评审结论须以「🔬 RES 研究员评审」独立留痕于 PR）'
    );
  ...
  if (!/8\/12 文档\(DEV\+PO，按当次需求开发\/bug 整改同步修改完善项目相关文档\)/.test(prompt))
    errors.push(
      '缺少「8/12 文档-按当次需求/bug 整改同步文档」门禁（流水线 8/12 文档须按本次需求开发/bug 整改同步修改完善项目相关文档）'
    );
  ...
  // 7/12 测试通过后必须做需求覆盖度检查（逐条需求 ↔ 开发落点 ↔ 测试覆盖），完成后经 ⏸CP2.5 暂停待用户确认
  if (!/需求覆盖度检查/.test(prompt))
    errors.push(
      '缺少「需求覆盖度检查」门禁（7/12 测试通过后须逐条核对开发落点/测试覆盖是否覆盖全部需求）'
    );
  ...
  // 用户要求"再加上PR合并（在合并前需暂停待用户确认）"。因此 9/12 PR 合并前必须停到 ⏸CP3：
  ...
  for (const frag of RELAY_MERGE_CORE) {
    ...
    if (idx === -1)
      errors.push(
        `合并确认缺少完整句式「${frag}」（9/12 PR 合并前必须暂停待用户确认，防未确认就合并/假装已合并）`
      );
  ...
  // 以及「确认合并/继续」放行句、禁止未确认就合并/假装已合并句、9/12 合并召唤话术示例（段内强校验）
  ...
  if (
    ...
    (!mergeCardSection.includes('合并前置状态') ||
      ...
      !mergeCardSection.includes('只有用户本人回复「确认合并/继续」才放行执行 9/12 PR 合并') ||
      !mergeCardSection.includes('禁止代替用户确认、禁止未获确认就合并、禁止假装已合并') ||
      !mergeCardSection.includes('收到确认前不得执行合并，不得输出 9/12 内容') ||
      !mergeCardSection.includes('「确认合并/继续」→ 放行 9/12 PR 合并') ||
      ...
      !/@CodeBuddy 接力 NPC_TEAM skill，执行 9\/12 PR 合并：/.test(mergeCardSection))
  )
    errors.push(
      '合并确认卡缺少必要要素（须含合并前置状态：评审清零/测试通过/CI success、用户确认才放行、收到确认前不得执行合并、确认合并/继续→放行映射、用户命令三选一完整（继续/补充/停止）、禁止未确认就合并/假装已合并、9/12 合并召唤话术示例）'
    );
  // CR 第 18 轮修复（追加 9 轮循环 R8）：合并确认卡段内补「9/12 → 10/12 接力衔接」强校验——
  // 此前 9.5 只校验 9/12 合并召唤话术，合并棒完成后如何进入 10/12 发布（独立发布棒召唤）无校验，
  // 若该链路描述被删/改 CI 不拦截，接力模式下合并后无法规范进入发布阶段。
  if (
    mergeCardSection !== null &&
    (!/@CodeBuddy 接力 NPC_TEAM skill，执行 10\/12 发布：/.test(mergeCardSection) ||
      !/9\/12 合并棒完成后/.test(mergeCardSection) ||
      !/10\/12 发布棒由 DEV 执行四要素/.test(mergeCardSection))
  )
    errors.push(
      '合并确认卡缺少 9/12→10/12 接力衔接（须含：9/12 合并棒完成后输出【接力卡·9/12】、10/12 发布召唤话术 @CodeBuddy 接力 NPC_TEAM skill，执行 10/12 发布：、10/12 发布棒由 DEV 执行四要素）'
    );
  // 铁律 13 禁止细节强校验（CR 第 2 轮修复：防铁律 13 被删空壳仍通过）
  if (
    rule13Section !== null &&
    (!rule13Section.includes('只有用户本人回复「确认合并/继续」才放行执行 9/12 合并') ||
      !rule13Section.includes('禁止代替用户确认、禁止未获确认就合并、禁止假装已合并') ||
      !rule13Section.includes('未真实 merge-pull 不得宣称已合并'))
  )
    errors.push(
      '铁律 13 缺少禁止细节（须含：只有用户回复「确认合并/继续」才放行 9/12 合并、禁止代替用户确认/未获确认就合并/假装已合并、未真实 merge-pull 不得宣称已合并）'
    );

  // ---- 9.6 发布真实执行（Issue #76 最新补充：发布形成四要素）----
  // 用户要求"发布（更新版本号、发布形成changelog、发布产物）" + "补充：发布（补充形成Release Notes）"。
  // 因此 10/12 发布必须真实执行四要素并留痕：① 更新版本号 ② 形成 CHANGELOG ③ 发布产物 ④ 形成 Release Notes；
  ...
  for (const frag of RELAY_RELEASE_CORE) {
    ...
    if (idx === -1)
      errors.push(
        `发布四要素缺少完整句式「${frag}」（10/12 发布必须真实执行版本号/CHANGELOG/发布产物/Release Notes 四要素并留痕）`
      );
  ...
  // 背景：用户要求"6/12 评审-修复循环自动进行，不用暂停、采用接力模式（自动继续），每步独立调用 @CodeBuddy"；
  ...
  for (const { f, scope } of searchTargets) {
    ...
    if (idx === -1)
      errors.push(
        `评审-修复自动连续循环缺少完整句式「${f}」（6/12 评审-修复循环须自动连续执行，且每一步（评审/修复/复评）在 PR 分别留痕，留痕以「每步」为粒度而非「每轮」，防假装进行）`
      );
  ...
  if (loopCardSection !== null) {
    ...
    for (const f of LOOP_CARD_CORE) {
      ...
      if (idx === -1)
        errors.push(
          `评审-修复循环留痕卡缺少完整句式「${f}」（6/12 循环内每步留痕/计轮口径/转 7/12 语义须完整，防提前放行）`
        );
  ...
  // 是自动连续循环审计产物。卡名自身须强校验——LOOP_CARD_CORE 的「转 7/12」「每步留痕必须真实可核实」
  ...
  // ① 提示词【流水线】段 ⏸CP3 合并确认→9/12 PR 合并→10/12 发布 整段被删；
  // ② 工作流程 ⏸ CP3 合并确认 / 阶段 9/12 PR 合并 / 阶段 10/12 发布 三行被删；
  ...
  // CR 第 6 轮修复：三节点除存在性校验外增加顺序校验（⏸CP3 合并确认 → 9/12 PR 合并 → 10/12 发布 递增 indexOf）
  const pipelineSection = sliceSection('【流水线（12 阶段 + 4 暂停点', '【任务书');
  if (pipelineSection !== null) {
    const pipelineFrags = ['⏸CP3 合并确认', '9/12 PR 合并', '10/12 发布'];
    ...
    if (pipelineBroken)
      errors.push(
        '提示词【流水线】段缺少合并/发布节点或顺序错乱（须含 ⏸CP3 合并确认 → 9/12 PR 合并 → 10/12 发布 完整主链，防主流程丢失合并/发布阶段）'
      );
  }
  ...
  if (
    ...
    (!workflowSection.includes('⏸ CP3 合并确认') ||
      !workflowSection.includes('阶段 9/12 PR 合并') ||
      !workflowSection.includes('阶段 10/12 发布'))
  )
    errors.push(
      '工作流程缺少合并/发布阶段行（须含 ⏸ CP3 合并确认 / 阶段 9/12 PR 合并 / 阶段 10/12 发布，防执行细化丢失合并/发布）'
    );
  // R5-1 修复：工作流程 7/12 需求覆盖度检查行（文档级）——新增需求覆盖度检查 + ⏸CP2.5 后
  ...
  if (
    workflowSection !== null &&
    (!/阶段 9\/12 PR 合并（PM）→ \*\*真实 merge-pull\*\*/.test(workflowSection) ||
      !/合并前 CI 须 success/.test(workflowSection) ||
      !/留痕合并结果/.test(workflowSection) ||
      !/阶段 10\/12 发布（DEV）→ \*\*四要素\*\*/.test(workflowSection) ||
      !/每一步（评审\/修复\/复评）均须在 PR 中分别回复留痕，留痕以「每步」为粒度而非「每轮」/.test(
        workflowSection
      ) ||
      !/阶段 8\/12 文档更新（DEV\+PO）→ \*\*按本次需求开发 \/ bug 整改/.test(workflowSection))
  )
    errors.push(
      '工作流程合并/发布/评审/文档行语义不完整（合并行须含：真实 merge-pull、合并前 CI 须 success、留痕合并结果；发布行须含：**四要素**；评审行须含：每一步（评审/修复/复评）均须在 PR 中分别回复留痕，留痕以「每步」为粒度而非「每轮」；文档行须含：按本次需求开发/bug 整改同步修改完善项目相关文档）'
    );
  ...
  // R7-1 修复：门禁表需求覆盖度检查行（文档级）——需求覆盖度检查是与合并/发布同级的质量门禁，
  // 防门禁表未补需求覆盖度行而 CI 不拦截（文档防幻觉清单与新增门禁脱节）。
  if (gateSection !== null && !gateSection.includes('需求覆盖度检查（⏸CP2.5）'))
    errors.push('门禁表缺少「需求覆盖度检查（⏸CP2.5）」门禁行');
  ...
  if (cardSection2) {
    ...
    if (!/PM.*⏸CP3 合并确认.*9\/12 PR 合并/s.test(cards))
      errors.push('角色卡片 PM 缺少合并职责（须含 主持 ⏸CP3 合并确认、执行 9/12 PR 合并）');
    if (!/DEV.*10\/12 发布执行/s.test(cards))
      errors.push('角色卡片 DEV 缺少发布职责（须含 10/12 发布执行）');
  ...
  // 第 11 轮将 R 计轮口径统一为「三棒同轮、复评未清零转第 R+1 轮、清零独立召唤 7/12」，
  ...
  // 已清零分支硬性要求完整句（已清零（且已达至少 10 轮）→ 独立召唤 7/12 测试）。
  ...
  if (
    ...
    (!smokeCSection.includes('同一次召唤内自动连续执行') ||
      ...
      !/已清零（且已达有效下限轮次 max\(10, 用户要求 N\)）→ 独立召唤 7\/12 测试/.test(
        smokeCSection
      ) ||
      !/中途不暂停、无需逐棒手动召唤/.test(smokeCSection))
  )
    errors.push(
      '冒烟测试方法 C 未同步新语义（须含：同一次召唤内自动连续执行；每一步（评审/修复/复评）在 PR 分别回复留痕，留痕以「每步」为粒度而非「每轮」；复评未清零 → 自动继续下一轮；已清零（且已达有效下限轮次 max(10, 用户要求 N)）→ 独立召唤 7/12 测试；中途不暂停、无需逐棒手动召唤）'
    );
  ...
  // ⑨ 冒烟测试方法 F（需求覆盖度检查验证）——R8-1 修复新增：
  // 方法 F 声明需求覆盖度检查 + ⏸CP2.5 确认全覆盖后才进入文档，是需求覆盖门禁的端到端验收标准。
  const smokeFSection = sliceContentSection(
    '**方法 F（需求覆盖度检查验证）**',
    '### 2. 冒烟测试（端到端，2 分钟）'
  );
  if (
    smokeFSection !== null &&
    (!smokeFSection.includes('需求覆盖度检查') ||
      !smokeFSection.includes('⏸CP2.5 需求覆盖确认') ||
      !smokeFSection.includes('覆盖度矩阵') ||
      !/用户确认需求全覆盖后才进入 8\/12 文档/.test(smokeFSection))
  )
    errors.push(
      '冒烟测试方法 F 缺少需求覆盖度检查语义（须含：需求覆盖度检查 / 覆盖度矩阵 / ⏸CP2.5 需求覆盖确认 / 用户确认需求全覆盖后才进入文档）'
    );

  // ⑧ 冒烟测试方法 D（合并确认接力）——CR 第 25 轮修复（本轮 5 轮循环 R3/W2）新增：
  // 方法 D 是 ⏸CP3 合并确认的端到端验收标准：
  // ① 未获用户回复「确认合并/继续」绝不执行 9/12 合并（铁律 13 在冒烟测试层的落点）；
  // ② 用户确认后把合并召唤话术发给下一次召唤 → 9/12 棒真实 merge-pull 并留痕（接力模式独立召唤语义）。
  ...
  if (
    smokeDSection !== null &&
    (!/未获用户回复「确认合并\/继续」绝不执行 9\/12 合并/.test(smokeDSection) ||
      !/把合并召唤话术发给下一次召唤/.test(smokeDSection) ||
      !/真实 merge-pull 并留痕/.test(smokeDSection))
  )
    errors.push(
      '冒烟测试方法 D 缺少合并确认接力语义（须含：未获用户回复「确认合并/继续」绝不执行 9/12 合并；用户确认后把合并召唤话术发给下一次召唤 → 9/12 棒真实 merge-pull 并留痕合并结果）'
    );
