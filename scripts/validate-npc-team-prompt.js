...
if (!m) {
  ...
  if (!cardSection) {
    ...
    // R6-1 修复：QA 卡片需求覆盖度检查职责（与主链 6/12、工作流程同步），
  ...
  if (!/7\/12 文档\(DEV\+PO，按当次需求开发\/bug 整改同步修改完善项目相关文档\)/.test(prompt))
    errors.push(
      '缺少「7/12 文档-按当次需求/bug 整改同步文档」门禁（流水线 7/12 文档须按本次需求开发/bug 整改同步修改完善项目相关文档）'
    );
  ...
  if (
    ...
    (!/阶段 8\/12 PR 合并（PM）→ \*\*真实 merge-pull\*\*/.test(workflowSection) ||
      ...
      !/阶段 7\/12 文档更新（DEV\+PO）→ \*\*按本次需求开发 \/ bug 整改/.test(workflowSection))
  )
    errors.push(
  ...
  if (
    ...
    (!smokeFSection.includes('需求覆盖度检查') ||
      ...
      !/用户确认需求全覆盖后才进入 7\/12 文档/.test(smokeFSection))
  )
    errors.push(
