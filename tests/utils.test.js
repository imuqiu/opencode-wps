...
function setPS(key, value) {
  mockStorage[key] = value;
}

var CONFIG = {
  get apiUrl() {
    return getPS('opencode_api_url') || 'http://127.0.0.1:14096';
  },
  set apiUrl(val) {
    setPS('opencode_api_url', val);
  },
  get cwd() {
    return getPS('opencode_cwd') || '';
  },
  set cwd(val) {
    setPS('opencode_cwd', val);
  },
  get agent() {
    return getPS('opencode_agent') || 'wps-expert';
  },
  set agent(val) {
    setPS('opencode_agent', val);
  },
};
...
test('CONSTANTS: 数值合理性', function () {
  assertTrue(
    CONSTANTS.MAX_RETRIES >= 1 && CONSTANTS.MAX_RETRIES <= 10,
    'MAX_RETRIES 应在 1-10 范围'
  );
