...
3. **wps-proofread** - **文档校对专项**（用户请求"校对/纠错/错别字/语病"时，必须优先加载本 Skill，不得自行跳过流程）
4. **wps-excel** - 处理 Excel 表格相关任务
5. **wps-ppt** - 处理 PPT 演示文稿相关任务

在执行任务前，先判断任务类型并优先使用对应的 skill。

> **校对路由（重要）**：当用户请求对文档进行校对（含"校对""检查错别字""语病""审校""proofread"等表述）时，**必须调用 `wps-proofread` Skill**，按其铁律与批次流程执行，严禁跳过。校对不等同于普通 Word 编辑，不得用 `wps-word` 的通用编辑流程替代。

> **校对铁律（防幻觉）**：校对报告必须由 `generateProofreadReport` 服务端根据真实累计的数据生成并落盘，**严禁用 write 手动拼造校对报告**。每批校对须上报 `_processed_to_paragraph` 真实进度（串行模式须严格递增），未覆盖全文不得声称"全部已修复"。
