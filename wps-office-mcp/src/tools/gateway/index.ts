...
const VERIFIED_TOOLS = new Set([
  ...
  'setFont', 'setTextColor', 'setParagraph', 'applyStyle', 'generateTOC',
  ...
  'setLineSpacing', 'insertSectionBreak',
  'switchDocument', 'getOpenDocuments',
  // === Word: Proofreading ===
  'enableTrackChanges', 'getTrackChangesStatus', 'replaceInParagraph', 'proofreadBasic', 'confirmBatchAiProofread',
  // === Excel ===
  'getActiveWorkbook', 'getCellValue', 'setCellValue', 'getRangeData', 'setRangeData',
  'setFormula', 'getFormula', 'setArrayFormula', 'diagnoseFormula', 'evaluateFormula', 'autoSum',
  ...
  'refreshLinks', 'consolidate', 'calculateSheet', 'getExcelContext', 'generateFormula', 'setZoom',
  ...
  'setSlideSize', 'setSlideTheme', 'setFontColor', 'setShapeFill',
...
const COM_ACTIONS: ToolIndexItem[] = [
  ...
  { name: 'setTextColor', description: '设置文字颜色', keywords: ['颜色', '文字', '字体颜色'], category: 'word', appType: WpsAppType.WRITER, paramsSchema: { color: { type: 'string', description: '颜色值', required: true } } },
  ...
  { name: 'insertSectionBreak', description: '插入分节符', keywords: ['分节'], category: 'word', appType: WpsAppType.WRITER, paramsSchema: { breakType: { type: 'string', description: '分节符类型: nextPage/continuous/evenPage/oddPage', required: false } } },
  { name: 'setLineSpacing', description: '设置行距', keywords: ['行距'], category: 'word', appType: WpsAppType.WRITER, paramsSchema: { lineSpacing: { type: 'number', description: '行距倍数', required: true }, paragraphIndex: { type: 'number', description: '段落索引', required: false } } },
  ...
  { name: 'proofreadAccumulate', description: '累加本批校对问题到会话 Map（五维评分报告用）', keywords: ['校对', '报告', '累加', '会话', 'accumulate'], category: 'word', appType: WpsAppType.WRITER, paramsSchema: { session_id: { type: 'string', description: '校对会话ID（UUID v4），整个流程保持一致', required: true }, issues: { type: 'array', description: '本批校对发现的问题列表', required: true }, doc_info: { type: 'object', description: '文档信息（仅首次调用需要）', required: false }, total_revisions: { type: 'number', description: '当前累计修订数（可选）', required: false } } },
  { name: 'generateProofreadReport', description: '生成五维评分校对报告', keywords: ['校对', '报告', '生成', '五维', '评分'], category: 'word', appType: WpsAppType.WRITER, paramsSchema: { session_id: { type: 'string', description: '校对会话ID', required: true }, output_file: { type: 'string', description: '报告输出文件路径（可选）', required: false } } },
  ...
  { name: 'evaluateFormula', description: '计算公式结果', keywords: ['公式', '计算'], category: 'excel', appType: WpsAppType.SPREADSHEET, paramsSchema: { formula: { type: 'string', description: '公式', required: true }, cell: { type: 'string', description: '目标单元格', required: false } } },
  { name: 'autoSum', description: '自动求和', keywords: ['求和', '合计'], category: 'excel', appType: WpsAppType.SPREADSHEET, paramsSchema: { range: { type: 'string', description: '数据区域', required: true }, targetCell: { type: 'string', description: '目标单元格', required: true } } },
  ...
  { name: 'setCellFormat', description: '设置单元格格式', keywords: ['格式'], category: 'excel', appType: WpsAppType.SPREADSHEET, paramsSchema: { range: { type: 'string', description: '要设置格式的范围，如 A1:C10、B2:D5', required: true }, format: { type: 'object', description: '格式设置对象，可包含 bold(粗体)、italic(斜体)、fontSize(字号)、fontName(字体名)、fontColor(字体颜色，如#FF0000)、bgColor(背景颜色)、underline(下划线)、strikethrough(删除线)、horizontalAlignment(水平对齐: left/center/right)、verticalAlignment(垂直对齐: top/center/bottom)、wrapText(自动换行)、numberFormat(数字格式)', required: true }, sheet: { type: 'string', description: '工作表名称，不填则使用当前活动工作表', required: false } } },
  ...
  { name: 'setZoom', description: '设置工作表缩放比例', keywords: ['缩放', '比例'], category: 'excel', appType: WpsAppType.SPREADSHEET, paramsSchema: { percent: { type: 'number', description: '缩放比例（10-400）', required: true } } },
  ...
  { name: 'setShapeFill', description: '设置形状填充', keywords: ['填充', '颜色'], category: 'ppt', appType: WpsAppType.PRESENTATION, paramsSchema: { slideIndex: { type: 'number', description: '幻灯片索引', required: false }, shapeName: { type: 'string', description: '形状名称', required: false }, fillColor: { type: 'string', description: '填充颜色', required: false } } },
  ...
  { name: 'setSlideSize', description: '设置幻灯片大小', keywords: ['大小', '尺寸'], category: 'ppt', appType: WpsAppType.PRESENTATION, paramsSchema: { width: { type: 'number', description: '宽度', required: false }, height: { type: 'number', description: '高度', required: false } } },
  { name: 'setSlideTheme', description: '设置幻灯片主题', keywords: ['主题'], category: 'ppt', appType: WpsAppType.PRESENTATION, paramsSchema: { theme: { type: 'string', description: '主题路径或名称', required: true } } },
  { name: 'setFontColor', description: '设置字体颜色', keywords: ['颜色', '字体'], category: 'ppt', appType: WpsAppType.PRESENTATION, paramsSchema: { slideIndex: { type: 'number', description: '幻灯片索引', required: false }, shapeName: { type: 'string', description: '形状名称', required: false }, color: { type: 'string', description: '颜色值', required: true } } },
