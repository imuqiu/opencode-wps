...
/**
 * 单批段落上限（R5-1 评审：与 getDocumentParagraphs 单次 200 段上限对齐，三处共享）
 * - getDocumentParagraphs 单次请求 ≤ 200 段（governance P1 拦截）
 * - SKILL.md 批次大小硬性规则：end - start + 1 ≤ 200
 * - proofreadAccumulate 串行单批进度增量 ≤ 200（防跳号假进度，R2-1）
 */
export const PROOFREAD_BATCH_MAX = 200;
...
export interface BatchAllocation {
  ...
  stepsLog: StepRecord[];
  /**
   * 是否服务端自动生成（Issue #179 阶段1 服务端自动分批）：
   * - true：由 generateAutoBatches 在会话首次初始化时自动切段生成，报告门禁按
   *   「串行进度」判定（processedToParagraph >= totalParagraphs），不要求批次逐一 done；
   * - false/undefined：AI（规划 agent）手动登记的编排批次，报告门禁按
   *   「全部批次 done + 凭证完整 + 区间覆盖全文」判定。
   */
  autoGenerated?: boolean;
}

/**
 * 服务端自动分批（Issue #179 阶段1，取代 planner subagent 的分批职责）
 *
 * 背景：4-subagent 重构中「分批规划」本应由 planner subagent 完成，但真实会话中
 * planner 从未被触发，_batch_allocations 从未登记 → 批次表为空 → 报告把「从未规划」
 * 误判为「全部完成」。分批本质是无脑连续切段，服务端一行循环即可，比让模型「自觉规划」
 * 可靠得多。本函数在会话首次初始化（_batch_allocations 为空）时自动生成连续批次并落盘，
 * 使批次表**永远非空**，「空批次表 = 全部完成」的误判根除。
 *
 * @param totalParagraphs 文档总段数（>=1）
 * @param batchSize 每批段数（默认 100，与 SKILL 推荐一致；<1/非有限值时回退 100）
 * @returns 连续批次数组（start>=1、end<=totalParagraphs、互不重叠、覆盖全文、autoGenerated=true）；
 *   totalParagraphs 非法（<1/非有限值）时返回空数组
 */
export function generateAutoBatches(
  totalParagraphs: number,
  batchSize: number = 100
): BatchAllocation[] {
  if (!Number.isFinite(totalParagraphs) || totalParagraphs < 1) return [];
  // R1-1 修复（PR #181 评审）：Math.floor(batchSize) 后必须 >= 1，否则 (0,1) 区间
  // 会得到 size=0，while 循环 end=start-1、start 永不前进 → 死循环挂起。
  const size =
    Number.isFinite(batchSize) && Math.floor(batchSize) >= 1 ? Math.floor(batchSize) : 100;
  const allocations: BatchAllocation[] = [];
  let start = 1;
  let idx = 1;
  // R5-3 评审：batchId 统一为 `auto-batch-N` 前缀——手动登记的批次**不得复用该前缀**，
  // 避免与自动批次混淆（autoGenerated 标记是权威判定，但命名冲突影响可读性/审计）。
  while (start <= totalParagraphs) {
    const end = Math.min(start + size - 1, totalParagraphs);
    allocations.push({
      batchId: `auto-batch-${idx}`,
      range: { start, end },
      status: 'pending',
      stepsLog: [],
      autoGenerated: true,
