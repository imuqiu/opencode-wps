...
describe('proofread-store batch allocation & step-log', () => {
  ...
  // ==================== Issue #179 阶段1：服务端自动分批 generateAutoBatches ====================

  test('generateAutoBatches：100 段文档默认每批 100 段 → 1 批连续覆盖全文', () => {
    const allocs = generateAutoBatches(100);
    expect(allocs.length).toBe(1);
    expect(allocs[0].range).toEqual({ start: 1, end: 100 });
    expect(allocs[0].autoGenerated).toBe(true);
    expect(allocs[0].status).toBe('pending');
    expect(allocs[0].stepsLog).toEqual([]);
  });

  test('generateAutoBatches：250 段默认每批 100 → 3 批连续无重叠覆盖全文', () => {
    const allocs = generateAutoBatches(250);
    expect(allocs.length).toBe(3);
    expect(allocs[0].range).toEqual({ start: 1, end: 100 });
    expect(allocs[1].range).toEqual({ start: 101, end: 200 });
    expect(allocs[2].range).toEqual({ start: 201, end: 250 });
    // 区间不重叠且覆盖全文
    const sorted = allocs.map(a => a.range).sort((a, b) => a.start - b.start);
    let cursor = 1;
    for (const r of sorted) {
      expect(r.start).toBe(cursor);
      cursor = r.end + 1;
    }
    expect(cursor).toBe(251);
    // 全部 autoGenerated
    expect(allocs.every(a => a.autoGenerated === true)).toBe(true);
  });

  test('generateAutoBatches：自定义 batchSize=50 → 每批 50 段', () => {
    const allocs = generateAutoBatches(120, 50);
    expect(allocs.length).toBe(3);
    expect(allocs[0].range).toEqual({ start: 1, end: 50 });
    expect(allocs[1].range).toEqual({ start: 51, end: 100 });
    expect(allocs[2].range).toEqual({ start: 101, end: 120 });
  });

  test('generateAutoBatches：非法参数返回空数组（totalParagraphs<1 / NaN / batchSize<=0 回退 100）', () => {
    expect(generateAutoBatches(0)).toEqual([]);
    expect(generateAutoBatches(-5)).toEqual([]);
    expect(generateAutoBatches(NaN)).toEqual([]);
    // batchSize<=0/非有限值回退 100
    const allocs = generateAutoBatches(150, 0);
    expect(allocs.length).toBe(2);
    expect(allocs[0].range).toEqual({ start: 1, end: 100 });
    expect(allocs[1].range).toEqual({ start: 101, end: 150 });
  });

  test('generateAutoBatches：batchSize 在 (0,1) 区间不再死循环（R1-1 评审修复）', () => {
    // 回归（PR #181 第 1 轮评审 R1-1）：此前 Math.floor(0.5)=0 → size=0 → while 死循环挂起。
    // 修复后回退默认 100，正常返回连续批次。
    const allocs = generateAutoBatches(250, 0.5);
    expect(allocs.length).toBe(3);
    expect(allocs[0].range).toEqual({ start: 1, end: 100 });
    expect(allocs[1].range).toEqual({ start: 101, end: 200 });
    expect(allocs[2].range).toEqual({ start: 201, end: 250 });
  ...
  test('generateAutoBatches：batchSize 小数向下取整（如 1.5→1）且不产生 end<start 非法区间（R1-1 评审修复）', () => {
    const allocs = generateAutoBatches(5, 1.5);
    expect(allocs.length).toBe(5);
    allocs.forEach(a => {
  ...
  test('generateAutoBatches：批次可直接落盘且能被 loadBatchAllocations 读回（批次表永远非空）', () => {
    const s = sid('auto-batch-persist');
    const allocs = generateAutoBatches(30);
    expect(saveBatchAllocations(s, allocs)).toBe(true);
    const loaded = loadBatchAllocations(s);
    expect(loaded.length).toBe(1);
    expect(loaded[0].batchId).toBe('auto-batch-1');
    expect(loaded[0].autoGenerated).toBe(true);
    removeSessionFromDisk(s);
  });
