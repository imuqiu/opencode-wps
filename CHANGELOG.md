...
- **Issue #78 六诊：Left→Right 同向切换被宿主忽略，改 Floating→Right 强制重停靠（根治窗格上移 42px 覆盖功能区）** — 用户切 `fix/issue-78-taskpane-probe` 分支重装后遮挡仍存在（145/16/17 三张新截图）。像素级复检：145.png 功能区 y44-91 正常；16.png/17.png 窗格白色顶边仍从 y=50 开始（正常 y=92），功能区右侧被白色覆盖——**五诊修复无效的原因：窗格已处于 Right 停靠，`DockPosition` 0→2 是同向 docked 切换，宿主不重新计算窗口位置**。修复：① `forceTaskPaneRedraw` 恢复回调改用 **Floating(4)→Right(2)「脱离停靠→重新停靠」序列**——浮动状态强制宿主销毁停靠窗口、重建浮动窗口，再停靠时重新计算窗格矩形（含顶边 Y），与「切标签触发宿主重排」同源（用户实测切标签后头部即恢复）；读回 `DockPosition` 验证，宿主拒绝 Floating 时回退 0→2 序列保底；② `scheduleTaskPaneOpenRedraw` 新增**二次兜底重绘**（`TASKPANE_OPEN_REDRAW_RETRY_DELAY=1500ms`）——首次重绘（400ms+150ms）可能早于宿主完成首次窗口布局，1500ms 时宿主已稳定，补一次重绘提高自愈成功率；两次重绘共享守卫（等待期内用户操作则放弃）与防抖（`taskPaneRedrawPending`）；③ 探针新增 **P4d** 记录 Floating→Right 重停靠读回结果（实机可验证宿主是否接受浮动→停靠控制）。`tests/taskpane-dock.test.js` 新增 2 个六诊用例（55 → 57：P4d 探针读回 / 二次兜底重绘守卫放弃），全量 8 套件全绿

- **Issue #78 五诊：像素级证据锁定根因——任务窗格上移 42px 覆盖 WPS 功能区，新增强制重新停靠 + 停靠锁定** — 用户按四诊探针指引实测 3 张截图（首次打开/切标签恢复/重开），像素级对比分析确认：① 首次打开时任务窗格白色区从 y=50 开始（正常 y=92），**整体上移 42px ≈ WPS 功能区高度**；② 功能区区域（y44-91）在首次打开时全白无任何内容（切标签后有功能区图标/按钮），窗格直接盖住「开始/插入」功能区；③ 底部输入区 y820 三图一致——WebView 活着、问题在宿主窗口位置而非页面布局；④ 切标签/重开后恢复——宿主在窗口切换时重新计算窗格位置。修复：① `createTaskPane` 创建后 + `forceTaskPaneRedraw` 恢复回调中设置 `DockPositionRestrict=1`（msoCTPDockPositionRestrictNoChange）锁定停靠方向，防宿主/用户把窗格拖成浮动覆盖功能区，宿主不支持时静默降级；② `forceTaskPaneRedraw` 恢复回调中强制重新停靠（先切 Left(0) 再切回 Right(2)），复现「切标签触发宿主重排」行为；③ 探针新增 P4c 记录 `DockPositionRestrict` 锁定读回结果。`tests/taskpane-dock.test.js` 新增 4 个五诊用例（51 → 55），全量测试套件全绿

- **Issue #78 四诊：任务窗格诊断探针（先定位清楚再动手）** — 前 3 轮修复（#79 DockPosition / #83 三层防御 / #90 打开面板重绘）均未解决「头部被遮挡 + WPS 顶栏标签被压扁」，用户关键线索「新建 WPS 标签后切回即恢复」。本轮**不改修复行为**，先采集可观测数据区分「插件停靠问题」与「WPS 宿主 bug」：`main.js` 新增 `probeTaskPane`（P1 DockPosition 实际值 Right=2/Floating=4 关键判据 / P2 窗格 Width/Height / P3 Window 宿主句柄 / P4 强制复位 Right 读回验证宿主是否接受程序控制 / P4b DockPositionRestrict / P5 残留 id 检测），打开面板（首次创建/切换打开）后自动采集，结果写入 `PluginStorage('taskpane_probe')` + 内存 + console 留痕；`taskpane.html` 新增 P6 视口诊断（innerHeight/clientHeight/dpr + 头部元素几何 top/bottom/height，写入 `taskpane_probe_page`）；点「连接状态」按钮弹窗末尾汇总展示两区块，用户截图反馈即可定位根因。`tests/taskpane-dock.test.js` 新增 8 个探针用例（43 → 51），含宿主拒绝停靠控制/属性读抛异常容错/残留 id 重建/弹窗汇总含/无数据不输出区块，全量测试套件 + 语法门禁全绿
