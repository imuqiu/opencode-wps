...
>   **超界请求 + 部分返回（R2-1）**：若超界请求 `(1,200)` 但输出被截断只返回 `100/150`（文档实际 150），
>   治理层会识别为真截断（`batchTruncated=true`），但 `allBatchesComplete` 以**实际返回末段**为准不置位
>   （只到 100，未覆盖全文）——你仍可**用相同的 `(1,200)` 同批重试补齐** `101-150`，而不会被误判"已覆盖全文"拦截。
>   **前提：治理层能确定文档总段数**（`getActiveDocument` 直接给出，或从输出头行「共 N 段」兜底提取）。
>   若「总段数: 未知」且输出头行也不含「共 N 段」（罕见，格式异常），治理层无法区分「超界已全返」与
>   「请求内真截断」，会保守地按截断处理（R1-1 说明）——此时请优先用 `getActiveDocument` 确认总段数，
>   或改用更小批次并显式传对 `end_paragraph`，避免依赖此兜底判定。
...
>   **重试达上限的兜底（R3-3）**：若同批重试已达上限（`MAX_BATCH_RETRY_LIMIT`，默认 3 次）输出仍截断，
>   治理层放行 `proofreadBasic`（避免死锁），但**你应知悉当前文本可能未覆盖本批全部段落**——
>   请审慎校对，必要时降为更小批次（如 50 段/批）重新获取后再校对，绝不要在覆盖范围不完整的情况下出报告。
...
- 每批**必须真实调用** `proofreadBasic` 并处理其返回的 issue（P14/P12 已强制）。**仅 `getDocumentParagraphs` 视觉扫描 + 上报进度 ≠ 校对**（铁律 2/3）。
- `_processed_to_paragraph` 必须与本批实际获取的段落严格一致（P25），issue 必须是本批段落范围内的真实问题（P26）。
- 只有逐批累加到覆盖全文（`_processed_to_paragraph` ≥ `totalParagraphs`）后，才允许调用 `generateProofreadReport`（铁律 4）。严禁未覆盖全文就出报告。
